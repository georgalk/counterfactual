...
def training4():
    ...
    st.header("Question Training 4")
    st.markdown("""
    Suppose now you are faced with a situation in which you have no information about the relative proportions 
    of the three colors in the urn.

    In other words, you only know that there are 9 balls inside the urn with red, black, and yellow colors.

    You will be presented with three scenarios—hypotheses that specify different possible numbers of red balls in the urn, 
    each of which can be either true or false. 
    ...
    st.subheader(f"Scenario 2: There is 1 Red ball and 8 Black and Yellow balls - Stage {st.session_state['stage']}")

    col1, col2 = st.columns(2)

    with col1:
        image_path = st.session_state.get("ellsberg1", "default_image.png")
    ...
    probability_strings = [f"{i} out of 9 ({round((i/9)*100, 1)}%)" for i in range(10)]
